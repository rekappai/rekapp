...
export default function ArchiveClient({ articles, lang, t }: { articles: AA[]; lang: Lang; t: any }) {
  ...
  return (
    <>
      ...
      <button className={'arc-filter-mob-btn' + (activeCount>0?' active-filter':'')} onClick={() => setDrawer(o=>!o)}>
      ...
      {filtered.length === 0 && <div className="empty-state" style={{ textAlign:'center' }}>{t.archive.noResults}</div>}
                ...
                {dayArticles.map(a => {
                  ...
                  return (
                    <Link key={a.id} href={'/' + lang + '/article/' + a.meta_slug} className="archive-story">
                      {a.stocks?.symbol && <span className="archive-story-ticker">{a.stocks.symbol}</span>}
