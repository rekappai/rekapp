...
export async function GET() {
  ...
  for (const lang of LANGUAGES) {
    for (const page of staticPages) {
      urls.push(url(\`\${BASE_URL}/\${lang}\${page.path}\`, now, page.changefreq, page.priority))
    }
    for (const market of ACTIVE_MARKETS) {
      urls.push(url(\`\${BASE_URL}/\${lang}/markets/\${market}\`, now, 'daily', 0.6))
    }
  }
  ...
  if (articles) {
    for (const a of articles) {
      ...
      urls.push(url(\`\${BASE_URL}/\${a.lang_code}/article/\${a.meta_slug}\`, date, 'weekly', 0.8))
    }
  }
